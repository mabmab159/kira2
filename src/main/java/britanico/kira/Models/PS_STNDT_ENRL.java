...
@NamedNativeQueries(@NamedNativeQuery(name = "PS_STNDT_ENRL.ultimaClaseAprobada",
        query = "SELECT TOP 1 X.CRSE_ID, X.STRM, X.SESSION_CODE, X.CLASS_SECTION, X.CLASS_NBR, X.FEC_FIN_CL, CAST(X" +
                ".FECHA_MAX AS " +
                "DATE) AS FEC_MAX, CAST(X.FECHA_INSCR AS DATE) AS FEC_INSCR, PAT.STND_MTG_PAT, X.EARN_CREDIT FROM" +
                " (SELECT CT.CRSE_ID, CT.SESSION_CODE, CT.CLASS_SECTION, CT.CLASS_NBR, CT.STRM, " +
                "RE.FEC_FIN_CL, DATEADD(MONTH, CAST('6' AS INT), CT.END_DT ) AS FECHA_MAX, " +
                ...
                ".RQMNT_DESIGNTN='' WHERE IC.INSTITUTION = 'ACPBR' AND IC.EMPLID = '351123' AND IC.STDNT_ENRL_STATUS " +
