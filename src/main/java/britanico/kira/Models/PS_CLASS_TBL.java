...
@NamedNativeQueries(@NamedNativeQuery(name = "PS_CLASS_TBL.clasesRecomendadas",
        query = "SELECT A.CLASS_NBR AS NRO_CLASE, D.DESCR AS DESCR_CLASE, D.COURSE_TITLE_LONG AS DESCR_CURSO, A.CLASS_SECTION AS SECCION_CLASE, " +
                "A.STRM AS CICLO_LECTIVO, A.SESSION_CODE AS SESION, A.WAIT_CAP AS  " +
                "VACANTES, C.ITEM_AMT AS PRECIO, B.MON, B.TUES, B.WED, B.THURS, B.FRI, B.SAT, B.SUN, B.STND_MTG_PAT AS COD_HORA" +
                ",B.FACILITY_ID AS AULA FROM PS_CLASS_TBL A INNER JOIN PS_CLASS_MTG_PAT B ON (A.SESSION_CODE " +
